...
> 목록은 타이틀 화이트리스트(`Producer Price Index`)로 **PPI 상품·산업 시리즈만** 노출한다. 카테고리 트리 탐색에 섞여 적재된 비PPI 매크로 지표(기대인플레 `EXPINF*`·실질금리 `REAINTRATREARAT*`·물가압력 `STLPPM*`·`MICH`·CPI 등)는 제외한다. (DB에는 그대로 보존 — 화면 노출만 차단)
